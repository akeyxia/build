...
# execute "cp $sdkdir/rc/MLO /tmp/sdk/$$/boot/"
# execute "cp $sdkdir/rc/u-boot.img /tmp/sdk/$$/boot/"
execute "cp $sdkdir/rc/uenv*.txt /tmp/sdk/$$/boot/"

if [ ! -f $sdkdir/rc/filesystem/arago*sdk*multimedia*dra7*.tar.gz ]
then
	echo "ERROR: failed to find rootfs tar in $sdkdir/rc/filesystem"
else
	echo "Extracting filesystem on ${device}2 ..."
	root_fs=`ls -1 $sdkdir/rc/filesystem/arago*sdk*multimedia*dra7*.tar.gz`
	execute "tar zxf $root_fs -C /tmp/sdk/$$/rootfs"
	execute "cp $sdkdir/rc/mk-eMMC-boot.sh /tmp/sdk/$$/rootfs/home/root/"
	execute "cp $sdkdir/rc/mk-qspi-boot.sh /tmp/sdk/$$/rootfs/home/root/"
fi
