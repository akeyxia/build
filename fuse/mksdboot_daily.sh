...
execute "cp $sdkdir/install/dra7xx/boot/* /tmp/sdk/$$/boot/ -rf"

if [ ! -f $sdkdir/arago-glsdk-multimedia-image-dra7xx-evm.tar.gz ]
then
	echo "ERROR: failed to find rootfs tar in $sdkdir/filesystem"
else
	echo "Extracting filesystem on ${device}2 ..."
	root_fs=`ls -1 $sdkdir/arago-glsdk-multimedia-image-dra7xx-evm.tar.gz`
	...
	execute "cp $sdkdir/install/dra7xx/rootfs/* /tmp/sdk/$$/rootfs/ -rf"
fi
