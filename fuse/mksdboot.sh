...
execute "cp $sdkdir/image/install/dra7xx/boot/* /tmp/sdk/$$/boot/ -rf"

if [ ! -f $sdkdir/image/arago-glsdk-multimedia-image-dra7xx-evm.tar.gz ]
then
	echo "ERROR: failed to find rootfs tar in $sdkdir/filesystem"
else
	echo "Extracting filesystem on ${device}2 ..."
	root_fs=`ls -1 $sdkdir/image/arago-glsdk-multimedia-image-dra7xx-evm.tar.gz`
	execute "tar zxf $root_fs -C /tmp/sdk/$$/rootfs"
	execute "cp $sdkdir/mk-eMMC-boot.sh /tmp/sdk/$$/rootfs/home/root/"
	execute "cp $sdkdir/mk-qspi-boot.sh /tmp/sdk/$$/rootfs/home/root/"
	execute "cp $sdkdir/image/install/dra7xx/rootfs/* /tmp/sdk/$$/rootfs/ -rf"
fi
